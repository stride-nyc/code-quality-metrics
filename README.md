...
- **Optional:** When `ANTHROPIC_API_KEY` is set and the analyzed history is squashed, adds a COSMIC-inspired functional size estimate (E/X/R/W data movements) for up to `AI_ANALYSIS_MAX_COMMITS` commits. Squashed commits on the default branch represent complete PRs, the right unit for this approximation. Written to `cfp_results` in `local_metrics_summary.json` and rendered in the drift report. Informational only; no calibrated target.
...
When `ANTHROPIC_API_KEY` is set, the PR comment includes a COSMIC-inspired functional size section estimating E/X/R/W data movements from the diff. Informational only; no calibrated target established.
