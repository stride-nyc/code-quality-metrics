...
function renderMasthead(summary) {
  ...
  const verdictLabel = archetypeSuppressed ? 'suppressed' : archetype;
...
${granularityLine}
<p class="verdict" data-status="${escapeHtml(verdictStatus)}">${escapeHtml(verdictLabel)}: ${escapeHtml(verdictText)}</p>
</header>`;
}
...
function renderGranularityLine(summary) {
  ...
  const text = summary.history_granularity_override
    ? `${base} &mdash; overridden by --history; detection itself found ${escapeHtml(summary.history_granularity_detected)}`
    : base;
