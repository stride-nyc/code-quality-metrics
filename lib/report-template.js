...
function describeThreshold(entry) {
  const { direction, healthyBoundary, criticalBoundary, tier } = entry;
...
<p class="verdict" data-status="${escapeHtml(verdictStatus)}">${escapeHtml(archetype)}: ${escapeHtml(verdictText)}</p>
</header>`;
}
