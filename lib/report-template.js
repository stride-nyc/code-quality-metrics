...
function renderExclusionsSection(summary) {
  ...
  // The old label used the masthead's word for a set the masthead does not count. Rendered only
  // when non-empty, mirroring every other optional bullet in this section.
  if (branches && branches.length > 0) {
    bullets.push(`<li>Branches considered: ${branches.map(escapeHtml).join(', ')}</li>`);
  }
