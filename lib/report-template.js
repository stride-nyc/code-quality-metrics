...
function renderMastheadDetail(summary) {
  ...
  const shipmentLine = renderShipmentLine(summary);
 ...
 */
/**
 * Render the deployment frequency section when the summary includes
 * deployment_frequency_floor (opt-in via releaseTagPattern in .codemetrics.json).
 * Returns empty string when the field is absent or null.
 * @param {object} summary
 * @returns {string}
 */
/**
 * Render a one-line masthead sentence about commit shipment status (GitHub #107).
 * Shows how many analyzed commits are confirmed shipped vs. unconfirmed against
 * the default branch. Returns empty string when the field is absent.
 * @param {object} summary
 * @returns {string}
 */
function renderShipmentLine(summary) {
  if (typeof summary.shipped_commits_count !== 'number') return '';
  const { shipped_commits_count, unconfirmed_commits_count, default_branch } = summary;
  const branch = default_branch ? escapeHtml(default_branch) : 'default branch';
  if (unconfirmed_commits_count === 0) {
    return `<p class="masthead-shipment">All ${shipped_commits_count} analyzed commits confirmed shipped to ${branch}.</p>`;
  }
  return `<p class="masthead-shipment">${shipped_commits_count} of ${shipped_commits_count + unconfirmed_commits_count} analyzed commits confirmed shipped to ${branch}; ${unconfirmed_commits_count} unconfirmed (squash-merged commits appear unconfirmed even when shipped).</p>`;
}

function renderCfpSection(summary) {
  if (!Array.isArray(summary.cfp_results) || summary.cfp_results.length === 0) return '';
  const total = summary.cfp_results.reduce((sum, r) => sum + r.estimated_cfp_delta, 0);
  const rows = summary.cfp_results.map(r => {
    const { entries, exits, reads, writes } = r.estimated_cfp_breakdown;
    return `<tr><td><code>${escapeHtml(r.sha)}</code></td><td>${r.estimated_cfp_delta}</td><td>${entries}</td><td>${exits}</td><td>${reads}</td><td>${writes}</td></tr>`;
  }).join('\n');
  return `<section class="cfp-section">
<h2>Estimated Functional Size</h2>
<p>Total across ${summary.cfp_results.length} squashed commit${summary.cfp_results.length === 1 ? '' : 's'}: <strong>${total}</strong> data movements (E+X+R+W). Each squashed commit represents a complete PR.</p>
<table><thead><tr><th>Commit</th><th>Total</th><th>Entry</th><th>Exit</th><th>Read</th><th>Write</th></tr></thead>
<tbody>${rows}</tbody></table>
<p class="cfp-note">COSMIC-inspired approximation from diff analysis. Available on squashed history only. Not ISO 19761 compliant; boundary ambiguity means counts are approximate. Informational only, no calibrated target.</p>
</section>`;
}

function renderDeploymentFrequency(summary) {
  const df = summary.deployment_frequency_floor;
  if (!df) return '';

  const countLine = `${df.release_count} release event${df.release_count === 1 ? '' : 's'} detected`;
  const intervalLine = df.median_interval_days !== null
    ? `Median release interval: ${df.median_interval_days} day${df.median_interval_days === 1 ? '' : 's'}`
    : 'Insufficient history for interval (fewer than 2 release events)';
  const staleLine = df.days_since_last !== null
    ? `Days since last release: ${df.days_since_last}`
    : '';

  return `<section class="deployment-frequency">
<h2>Deployment Frequency (floor estimate)</h2>
<p>Counted from release tags or commit subjects matching <code>releaseTagPattern</code> in <code>.codemetrics.json</code>. A floor only: deleted tags and unrecognized conventions are invisible.</p>
<ul>
<li>${escapeHtml(countLine)}</li>
<li>${escapeHtml(intervalLine)}</li>
${staleLine ? `<li>${escapeHtml(staleLine)}</li>` : ''}
</ul>
</section>`;
}
...
${renderDeploymentFrequency(summary)}
${renderCfpSection(summary)}
...
module.exports = { renderReportHtml, fallbackFindings, formatValue, renderDeploymentFrequency, renderShipmentLine, renderCfpSection };
