...
function describeThreshold(entry) {
  const { direction, healthyBoundary, criticalBoundary } = entry;
  if (healthyBoundary == null && criticalBoundary == null) return null;
...
/**
 * Render the description block above a tile: what the metric measures and
 * how it connects to DORA, sourced from lib/metric-descriptions.js (which
 * traces to metrics-specification.md). Falls back to nothing rendered if a
 * catalog entry somehow has no matching description, rather than throwing,
 * since a missing description should not break the whole report.
 * @param {object} entry
 * @returns {string}
 */
...
function renderMetricGrid(catalog) {
  const tiles = catalog.map(entry => {
    const card = entry.hasGauge ? renderGaugeCard(entry) : renderStatCard(entry);
