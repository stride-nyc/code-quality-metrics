...
function renderTopSummary(catalog, summary) {
  ...
  if (bandedConcerns.length === 0) {
    ...
    const concernSentence = `${counts.join(' and ')} signal${bandedConcerns.length === 1 ? '' : 's'} against a calibrated threshold, `
    ...
    headline = bandedGoodCount > bandedConcerns.length
      ? `Most metrics scored against a calibrated threshold in this run are healthy. It still flags ${concernSentence}`
      : `This run flags ${concernSentence}`;
  }
