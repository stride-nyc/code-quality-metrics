...
function renderArchetypeSection(summary, catalog) {
  ...
  // archetypeSuppressed's bodyText already opens with "Archetype suppressed:", so no separate
  // label prefix is added here -- a label of "suppressed" would double the word. Otherwise, no
  // archetype value at all (the "could not be determined" fallback inside describeArchetypeBody)
  // also gets no label prefix, since there is no archetype name to attach one to.
  const verdictLabel = (!archetypeSuppressed && archetype) ? archetype : null;
  const verdictLine = verdictLabel ? `${escapeHtml(verdictLabel)}: ${escapeHtml(bodyText)}` : escapeHtml(bodyText);
  return `<section class="archetype-note">
<h2>Team archetype <span class="under-development">(under development)</span></h2>
<p class="archetype-disclaimer">The names are borrowed from DORA; the four-way grouping is not. DORA derives its own team archetypes from cluster analysis of survey responses covering burnout, friction and delivery instability, not from commit shape, and publishes no such grouping from commit history.</p>
...
function renderExclusionsSection(summary) {
  ...
  // code-quality-metrics-g39: the branch name list moved here from the masthead. Rendered only
  // when non-empty, mirroring every other optional bullet in this section.
  if (branches && branches.length > 0) {
    bullets.push(`<li>Branches analyzed: ${branches.map(escapeHtml).join(', ')}</li>`);
  }
