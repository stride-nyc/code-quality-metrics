...
function describeAllHealthy(bandedGoodEntries) {
  const phrases = bandedGoodEntries.map(describeGoodSignalPhrase);
  return `All signals are positive, indicating this run's commits sit within the healthy range of the reference benchmark on every calibrated metric measured: ${phrases.join('; ')}.`;
}
...
function renderTopSummary(catalog) {
  ...
  if (bandedConcerns.length === 0) {
    ...
    headline = bandedGoodCount > bandedConcerns.length
      ? `Most metrics scored against a calibrated threshold in this run are healthy. It also flags ${signalPhrase}, ${ledByPhrase}`
