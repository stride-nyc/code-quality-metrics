...
function renderArchetypeSection(summary, catalog) {
  ...
  // code-quality-metrics-m7kt: project_lifecycle: initial-build withholds large_commits_pct and
  // sprawling_commits_pct the same way (lib/report.js's WITHHELD_WHEN_GREENFIELD_KEYS) -- a
  // classification built from withheld inputs is not comparable, so it must be suppressed here
  // too, rather than fed to describeArchetypeBody, which previously read the withheld catalog
  // directly: a withheld entry's healthyBoundary is nulled and its direction rewritten to
  // 'informational', so archetypeSignalPhrase's higher-is-worse branch never matched and fell
  // through to the higher-is-better return, printing "fell below the healthy line at 48.89
  // (healthy at or above null)" -- wrong verb, a literal "null" boundary, and (from
  // ARCHETYPE_RULE_DESCRIPTIONS) a claimed critical-line crossing for a signal whose critical
  // band was withheld, not evaluated (measured live in flight-info-spike). Checked against each
  // archetype signal's own catalog entry -- any one of the four rewritten to 'informational' by
  // withholdEntry -- rather than re-deriving "was this withheld" from project_lifecycle alone,
  // so this stays correct even if the set of withheld keys changes without this function's
  // knowledge, and it composes with the history-granularity suppression above for the same
  // reason those two withholding passes already compose in buildMetricCatalog.
  const anyInputWithheld = ARCHETYPE_SIGNAL_KEYS.some(key => {
    const entry = catalog.find(e => e.key === key);
    return entry && entry.direction === 'informational';
  });
  const archetypeSuppressed = historySuppressed || anyInputWithheld;
  const bodyText = historySuppressed
    ...
    : summary.project_lifecycle === 'initial-build'
      ? 'Archetype suppressed: this looks like an initial build, and two of its four input signals (large commits, sprawling commits) have a withheld verdict for that reason, so the composite verdict has no valid inputs.'
      : anyInputWithheld
