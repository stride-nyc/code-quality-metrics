 ...
 * records separated by RECORD_SEPARATOR (git format `%H|%ci|%an|%B%x1e`, produced by
 ...
 * format string, not of this parser.
...
function getExpectedCommitCount(refs, sinceStr) {
  return refs.reduce((total, ref) => {
    const output = runGitCommand(`git rev-list --no-merges --count --since="${sinceStr}" ${ref}`);
    const count = parseInt(output, 10);
    return total + (Number.isNaN(count) ? 0 : count);
  }, 0);
}

module.exports = { runGitCommand, parseGitLog, isTestFile, analyzeCommit, getCommitDiff, detectHistoryGranularity, windowIncludesRepositoryRoot, findRepositoryRootShas, commitIntroducesProductionFiles, findEffectiveRootSha, getExpectedCommitCount };
