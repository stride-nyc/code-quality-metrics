...
const { isTestFile, isExcludedPath, isRepoFurniture, matchesAnyPattern, looksLikeTestPath } = require('./metrics');
  ...
  try {
    ...
    for (const line of statsLines) {
      ...
      if (isTestFile(filename)) {
        ...
        if (looksLikeTestPath(filename)) {
          suspectTestPaths.push(filename);
        }
      }
    }
...
/**
 * Count unique commits reachable from the given refs since a date boundary. All refs are
 * passed to a single `git rev-list --count` call so git deduplicates shared ancestry; a
 * per-ref loop and sum would count shared commits once per branch, producing a 21x overcount
 * on repositories with many branches that diverge late (code-quality-metrics-u2eq).
 *
 * Used as an independent cross-check against the commit count local-code-metrics.js's own
 ...
 * false discrepancy. A failed resolve contributes 0 rather than throwing, the same graceful
 * degradation runGitCommand already gives every other caller in this file.
 *
 * sinceStr is optional: omitting it (used by the --max-commits unbounded safety pre-flight
 * check in local-code-metrics.js) counts the refs' whole reachable history instead of a
...
function getExpectedCommitCount(refs, sinceStr) {
  ...
  const output = runGitCommand(`git rev-list --no-merges --count ${sinceArg}${refs.join(' ')}`);
...
/**
 * Fetch production release tags from git, filtered by releaseTagPattern and excluding
 * stagingTagPattern matches. Returns [] when releaseTagPattern is null (feature off).
 * Results are sorted ascending by date. This is the primary detection source for
 * deployment frequency (GitHub #65); commit-subject fallback lives in lib/metrics.js.
 *
 * @param {string|null} releaseTagPattern RegExp source, or null to disable
 * @param {string|null} stagingTagPattern RegExp source, or null for no exclusion
 * @returns {Array<{name: string, date: string}>}
 */
function fetchReleaseTags(releaseTagPattern, stagingTagPattern) {
  ...
  let output;
  try {
    output = execSync("git for-each-ref --format='%(refname:short)\t%(creatordate:iso)' refs/tags", { encoding: 'utf8' }).trim();
  } catch (err) {
    console.error(`fetchReleaseTags: git for-each-ref failed: ${err instanceof Error ? err.message : String(err)}`);
    return [];
  }
  ...
  for (const line of output.split('\n')) {
    ...
    if (!trimmed) continue;
    const [name, ...dateParts] = trimmed.split('\t');
    const dateStr = dateParts.join('\t').trim();
    if (!name || !dateStr) continue;
    if (!releaseRe.test(name)) continue;
    if (stagingRe && stagingRe.test(name)) continue;
    events.push({ name, date: new Date(dateStr).toISOString() });
  }
...
function findDefaultBranch() {
  try {
    const ref = execSync('git symbolic-ref --short refs/remotes/origin/HEAD', { encoding: 'utf8' }).trim();
    return ref.replace(/^[^/]+\//, '');
  } catch { /* no remote HEAD configured */ }
  for (const candidate of ['main', 'master']) {
    try {
      execSync(`git rev-parse --verify ${candidate}`, { encoding: 'utf8' });
      return candidate;
...
function fetchDefaultBranchShas(branchName, maxCount) {
  if (!branchName) return new Set();
  try {
    const output = execSync(`git rev-list ${branchName} --max-count=${maxCount}`, { encoding: 'utf8' }).trim();
    return new Set(output.split('\n').map(s => s.trim()).filter(Boolean));
  } catch (err) {
    console.error(`fetchDefaultBranchShas: git rev-list failed: ${err instanceof Error ? err.message : String(err)}`);
    return new Set();
  }
}

module.exports = { runGitCommand, parseGitLog, isTestFile, analyzeCommit, getCommitDiff, detectHistoryGranularity, findContentDuplicateGroups, windowIncludesRepositoryRoot, findRepositoryRootShas, commitIntroducesProductionFiles, findEffectiveRootSha, getExpectedCommitCount, findNewestCommitDate, fetchReleaseTags, findDefaultBranch, fetchDefaultBranchShas };
