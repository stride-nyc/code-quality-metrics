...
/**
 * Does a commit introduce at least one production file? Reuses the tool's existing
 * test/production/excluded classification (isTestFile, isExcludedPath from lib/metrics.js)
 * exactly the way analyzeCommit's own prodFiles count does -- no new path pattern, no tuned
 * constant such as repository age or commit count (code-quality-metrics-fex3, GitHub #71). A
 * file counts as production here under precisely the same rule analyzeCommit uses: not a test
 * file, not excluded by CONFIG.ANALYSIS_IGNORE_PATTERNS.
 *
 * CAVEAT: CONFIG.ANALYSIS_IGNORE_PATTERNS defaults to empty (lib/config.js), so a bare
 * LICENSE/README-only root commit -- stride-nyc/73V's actual root, ec1026c4 -- introduces a
 * "production" file under this same rule at default configuration: neither file is a test
 * file, and neither matches any pattern the repository has not configured. findEffectiveRootSha
 * below still walks forward correctly whenever a root's own classification does read zero (an
 * empty root commit, or one whose only files are test files, or a repository that has already
 * configured ANALYSIS_IGNORE_PATTERNS for its own non-code bootstrap paths); it does not, by
 * itself, reclassify LICENSE/README under bare defaults. See findEffectiveRootSha's own comment
 * and metrics-specification.md's Scaffold Root Commit Detection section for this same caveat.
 * @param {string} sha
 * @returns {boolean}
 */
function commitIntroducesProductionFiles(sha) {
  ...
  return output
    .split('\n')
    .filter(Boolean)
    .some(filename => !isExcludedPath(filename) && !isTestFile(filename));
}
...
function findEffectiveRootSha(rootSha) {
  if (commitIntroducesProductionFiles(rootSha)) return rootSha;

  const log = runGitCommand('git log --all --reverse --pretty=format:%H');
  ...
  for (const sha of candidates) {
    if (commitIntroducesProductionFiles(sha)) return sha;
  }
  return rootSha;
}

module.exports = { runGitCommand, parseGitLog, isTestFile, analyzeCommit, getCommitDiff, detectHistoryGranularity, windowIncludesRepositoryRoot, findRepositoryRootShas, commitIntroducesProductionFiles, findEffectiveRootSha };
