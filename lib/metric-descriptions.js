...
const METRIC_DESCRIPTIONS = {
  ...
  duplication_semantic_findings: {
    measures: 'Duplicate logic Claude identifies (Layer 2, semantic detection) that static line-matching misses, such as the same behavior implemented with different variable names or structure. Reports "Not measured" rather than 0 whenever no usable answer came back, either because the layer never ran (no ANTHROPIC_API_KEY set) or because it ran and the call failed or its response was truncated. A missing measurement is not the same thing as a real zero-findings result.',
