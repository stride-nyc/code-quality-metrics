...
const METRIC_DESCRIPTIONS = {
  large_commits_pct: {
    measures: 'How often a commit is big enough that nobody realistically read it line by line. A commit this size is harder to review carefully, but where the line falls is a selectivity choice, not a validated health boundary, and the largest commits in the published literature are disproportionately vendored imports, license sweeps, and generated documentation, a pattern this project\'s own calibration data hit too.',
    dora: 'Working in small batches is one of the practices DORA ties most directly to healthy delivery.'
  },
  sprawling_commits_pct: {
    measures: 'How often a single commit touches a lot of files at once. It is used as a proxy for a fix rippling through unrelated files by pattern rather than by understanding it, but the research behind this threshold found it sits close to automatic for most commits regardless of practice, and that the largest, most file-spanning commits tend to be license sweeps, generated documentation, and merges rather than drift.',
  ...
  uncovered_prod_rate: {
    measures: 'Large changes to production code that shipped without touching a single test. It is built from the large-commit check and the test/prod co-change check above, so it carries whatever uncertainty each of those carries on its own.',
  ...
  message_quality_pct: {
    measures: 'How many commit messages meet a word-count and format bar meant to signal a message that explains what changed. In practice this mostly tracks whether the project has adopted the Conventional Commits format rather than whether any single message actually explains anything.',
    dora: 'One of the version control signals DORA tracks.'
  },
  test_coverage_rate: {
    measures: 'Commits that changed tests and production code together. This cannot distinguish test-first from test-after: a team that deliberately lands a failing test and its implementation as two separate commits looks identical, by this count, to a team that never wrote the test.',
    dora: 'Automated testing is not one of the seven capabilities in DORA\'s AI model, so this metric rests on general engineering practice rather than a DORA finding. The DORA capabilities this toolkit can observe are strong version control and working in small batches.'
  },
  ...
  avg_lines_changed: {
    ...
    dora: 'This average sits on a heavy-tailed distribution, so a handful of huge commits can dominate it; a real shift toward the small batches DORA ties to healthy delivery shows up more reliably in the percentile below than in this figure.'
  },
  p90_lines_changed: {
  ...
  p90_files_changed: {
    ...
    dora: 'Sprawling commit percentage above already ties file-spanning changes to DORA\'s measured 7.2% instability increase per 25% rise in AI adoption; this shows how far the spread stretches past that threshold, not a separate DORA finding.'
  },
  test_isolation_rate: {
    measures: 'Commits that changed only tests, with no production code alongside. That can mean a failing test written before its implementation, or tests added or improved on their own; it can just as easily mean a test was deleted or disabled. The count does not distinguish between them.',
    dora: 'Automated testing is not one of the seven capabilities in DORA\'s AI model, so this metric rests on general engineering practice rather than a DORA finding. The DORA capabilities this toolkit can observe are strong version control and working in small batches.'
  },
