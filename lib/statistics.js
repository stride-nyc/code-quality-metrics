 ...
 * callers outside this module's scope still depend on them: `isOutlier` below uses them as an
 * internal heuristic cutoff (not a scored threshold), and local-code-metrics.js still writes
...
function computeStatistics(sizes, timestamps) {
  ...
  // mean/stddev: retained for isOutlier's internal cutoff and for backwards-compatible JSON
  // output only -- see this function's own JSDoc above for why neither is a valid population
