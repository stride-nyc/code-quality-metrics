...
/**
 * Linear-interpolation quantile of an already-sorted numeric array (same method
 * `computeStatistics` uses for p50/p90/p95, extracted so `isOutlier`'s fence can reuse it on a
 * log-transformed copy of the same data).
 ...
 * callers outside this module's scope still depend on them: local-code-metrics.js still writes
 * `stddev_lines_changed` into the summary JSON for backwards compatibility. `isOutlier` below no
 * longer reads either field (code-quality-metrics-496): a cutoff of mean + 2*stddev is dragged
 * toward whatever new extreme value is added to the window, which made the per-commit outlier
 * flag non-monotonic in the wrong direction -- adding one larger commit could push the cutoff
 * high enough to un-flag smaller commits that were already flagged, and the effect was strongest
 * on exactly the windows this toolkit exists to flag. Do not add a healthy/critical band keyed on
 * either field; see lib/thresholds.js's AVG_LINES_CHANGED removal-site comment and
 * metrics-specification.md's Metric 4 section. p50/p90/p95 are the statistics that should carry
 * any scored signal for this distribution.
 * @param {number[]} sizes
...
function computeStatistics(sizes, timestamps) {
  ...
  // isOutlier: Tukey's fence (Tukey, 1977) on log-transformed sizes, at the "far out" multiplier
  // (3 x IQR rather than the classic 1.5 x IQR "outlier" fence) -- see code-quality-metrics-496.
  // Q1/Q3 sit deep in the body of the distribution rather than near the tail, so a single added
  // extreme value shifts their interpolated rank position by a small, bounded amount (the two
  // neighboring body values either side of the quantile index), instead of being pulled toward
  // the new value's own magnitude the way mean + 2*stddev and a bare p95 both are. Logging first
  // matches the roughly log-normal body this distribution's own heavy tail sits on top of (see
  // metrics-specification.md's Metric 4 and Metric 5 sections), which is what makes Q1/Q3 spacing
  // meaningful here. The far-out (3x) multiplier is used, not the classic 1.5x fence, because 1.5x
  // sits close enough to ordinary body values on this toolkit's own measured windows to flicker
  // across the fence as new commits are added -- 3x does not, on every window measured so far.
  // This is a large empirical improvement over the old rule, not a proof that no input can invert
  // it; unlike mean/stddev, this fence is not reported anywhere and carries no back-compat need.
  const logSorted = sizes.map(v => Math.log(v + 1)).sort((a, b) => a - b);
  const q1Log = interpolatedQuantile(logSorted, 0.25);
  const q3Log = interpolatedQuantile(logSorted, 0.75);
  const iqrLog = q3Log - q1Log;
  const fenceLog = q3Log + 3 * iqrLog;
  const cutoff = Math.exp(fenceLog) - 1;
  return {
    p50: quantile(0.5),
    p90: quantile(0.9),
    p95: quantile(0.95),
