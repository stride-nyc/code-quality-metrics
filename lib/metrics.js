...
function classifyDoraArchetype(summary) {
  ...
  // message_quality_pct is deliberately not read here: MESSAGE_QUALITY_PCT carries no band in
  // THRESHOLDS (see lib/thresholds.js's removal-site comment) because the literature found no
  // defensible boundary for it. Scoring this archetype against an un-banded metric would
  // reinstate, inside this composite, the exact verdict the band's removal rejected
  // (code-quality-metrics-6vi).
  //
  // The four conditions below read each metric's own calibrated .healthy bound directly,
  // rather than a copy of it, so a recalibration of e.g. SPRAWLING_COMMITS_PCT moves this
  // boundary too instead of leaving a second, unsynced number behind -- the defect this fixes.
  ...
  // legacy-bottleneck reads each metric's own .critical bound directly, same rationale as
  // harmonious-high-achiever above.
  if (sprawling > SPRAWLING_COMMITS_PCT.critical && large > LARGE_COMMITS_PCT.critical) return 'legacy-bottleneck';
