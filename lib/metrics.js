...
function generateInsights(summary, metrics) {
  ...
  // Judged against counted_additions/counted_deletions (exclusion-scoped), not
  // total_additions/total_deletions (whole-diff), for the same reason lib/claude.js's
  // selectClaudeCommits does (code-quality-metrics-ce9m): a commit that is almost entirely
  // excluded vendored/generated content must not read as an addition-heavy AI-drift signal
  // on the strength of that excluded volume. Measured case (73V, cc7c77aa): 14,679 total
  // changed lines, 14,410 of them excluded terraform, 216 real production lines.
  const possibleAICommits = metrics.filter(m =>
    m.large_commit && m.counted_additions > m.counted_deletions * THRESHOLDS.AI_BATCH_SHARE.additionsRatio
  ).length;
...
/**
 * Smoke alarm: returns true if the path contains a segment that looks like a test
 * directory, regardless of whether isTestFile() already matched it. Deliberately
 * over-matches -- output is a warning for a human, never an input to a metric.
 * @param {string} filename
 * @returns {boolean}
 */
function looksLikeTestPath(filename) {
  return /(?:^|\/)(?:tests?|specs?|__tests__)(?:\/|$)/i.test(filename);
}

/**
 * Whether a string looks like a version reference (semantic version, numbered release).
 * Used to surface candidates that didn't match any configured release pattern.
 * @param {string} name
 * @returns {boolean}
 */
...
/**
 * Derive deployment frequency statistics from a sorted list of release events.
 * Returns null when events is empty (feature off or no releases detected).
 * All intervals and days_since_last are whole days (Math.round).
 * This is a floor, not a count: deleted tags and unrecognized conventions are invisible.
 *
 * @param {Array<{name: string, date: string}>} events sorted ascending by date
 * @param {string} asOf ISO date string used as "now" for days_since_last
 * @returns {{ release_count: number, median_interval_days: number|null, mean_interval_days: number|null, days_since_last: number } | null}
 */
function detectDeploymentFrequency(events, asOf) {
  ...
  for (let i = 1; i < events.length; i++) {
    const diff = new Date(events[i].date).getTime() - new Date(events[i - 1].date).getTime();
    intervals.push(Math.round(diff / msPerDay));
  }

  let median_interval_days = null;
  let mean_interval_days = null;
  if (intervals.length > 0) {
    const sorted = [...intervals].sort((a, b) => a - b);
    const mid = Math.floor(sorted.length / 2);
    median_interval_days = sorted.length % 2
      ? sorted[mid]
      : Math.round((sorted[mid - 1] + sorted[mid]) / 2);
    mean_interval_days = Math.round(intervals.reduce((s, v) => s + v, 0) / intervals.length);
  }

  const lastMs = new Date(events[events.length - 1].date).getTime();
  const days_since_last = Math.round((asOfMs - lastMs) / msPerDay);

  return { release_count: events.length, median_interval_days, mean_interval_days, days_since_last };
}

/**
 * Find tag/ref names that look like versions but match neither the production
 * release pattern nor the staging exclusion pattern. These are surfaced as
 * warnings: a convention change produces a lower floor rather than an error,
 * but naming the uncounted refs lets the operator notice.
 *
 * Returns empty array when releaseTagPattern is null (feature off).
 *
 * @param {string[]} names
 * @param {string|null} releaseTagPattern RegExp source, or null
 * @param {string|null} stagingTagPattern RegExp source, or null
 * @returns {string[]}
 */
function findUnmatchedVersionShapedRefs(names, releaseTagPattern, stagingTagPattern) {
  if (!releaseTagPattern) return [];
  const releaseRe = new RegExp(releaseTagPattern);
  const stagingRe = stagingTagPattern ? new RegExp(stagingTagPattern) : null;

  return names.filter(name => {
    if (releaseRe.test(name)) return false;
    if (stagingRe && stagingRe.test(name)) return false;
    return looksLikeVersion(name);
  });
}

function findReleaseCommitsFromSubjects(commits, releaseCommitSubjectPattern) {
  if (!releaseCommitSubjectPattern) return null;
  const re = new RegExp(releaseCommitSubjectPattern);
  return commits
    .filter(c => re.test(c.subject))
    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
}

module.exports = { isTestFile, isExcludedPath, isRepoFurniture, matchesAnyPattern, isAIAgentCommit, isBotCommit, scoreMessageQuality, classifyDoraArchetype, generateInsights, looksLikeTestPath, detectDeploymentFrequency, findUnmatchedVersionShapedRefs, findReleaseCommitsFromSubjects };
