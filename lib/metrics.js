...
/** Known git trailer keys (case-insensitive) that pad word count without adding prose. */
const KNOWN_TRAILER_KEYS = new Set([
  'pr-url', 'reviewed-by', 'signed-off-by', 'co-authored-by', 'fixes', 'refs',
  'closes', 'cc', 'acked-by', 'tested-by', 'reported-by', 'suggested-by',
  'helped-by', 'thanks-to',
]);

/** @type {RegExp} A `Key: value` line, matching git's own trailer line shape. */
...
function isTrailerLine(line) {
  const match = TRAILER_LINE_RE.exec(line.trim());
  return !!match && KNOWN_TRAILER_KEYS.has(match[1].toLowerCase());
}
...
function generateInsights(summary, metrics) {
  ...
  if (largePct > THRESHOLDS.LARGE_COMMITS_PCT.critical) {
  ...
  if (sprawlingPct > THRESHOLDS.SPRAWLING_COMMITS_PCT.critical) {
  ...
  if (uncoveredProdPct > THRESHOLDS.UNCOVERED_PROD_RATE.critical) {
  ...
  } else if (uncoveredProdPct >= THRESHOLDS.UNCOVERED_PROD_RATE.warning) {
  ...
  if (avgLines > THRESHOLDS.AVG_LINES_CHANGED.critical) {
  ...
  } else if (avgLines > THRESHOLDS.AVG_LINES_CHANGED.warning) {
