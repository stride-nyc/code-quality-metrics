...
/**
 * Return the top N commits by counted lines changed (counted_additions +
 * counted_deletions), sorted descending. Does not mutate the input array.
 *
 * Ranks on the exclusion-aware counted_* fields, not the raw whole-diff
 * total_additions/total_deletions, matching the four sites PR #100 already
 * fixed on this basis (lib/claude.js's pre-filter and sort, lib/metrics.js's
 * AI batch-share check, and net_additions_ratio in local-code-metrics.js): a
 * commit whose diff is mostly ANALYSIS_IGNORE_PATTERNS-excluded content
 * (a vendored sync, a generated-file dump) should not rank above a commit
 * with a genuinely larger production change just because its raw total is
 * bigger.
 * @param {Array<{counted_additions: number, counted_deletions: number}>} metrics
 * @param {number} [n]
 * @returns {Array<object>}
 */
...
module.exports = {
  ...
  groupForMetricKey,
  hasVerdict,
  // Exported for __tests__/thresholdProvenance.test.js's greenfield substitution provenance
  // gate only: that test needs the real map, not a source-text regex, to check that
  // test_coverage_rate/uncovered_prod_rate are never wired to THRESHOLDS.GREENFIELD_MODERN.
  GREENFIELD_SUBSTITUTED_KEYS
};
