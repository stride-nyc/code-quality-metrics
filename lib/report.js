...
function buildMetricCatalog(summary, duplicates) {
  ...
  // code-quality-metrics-31w: a separate pass, after squash withholding, not composed into the
  // same map -- the two conditions are independent and a window could in principle trip both.
  // Guarded on !entry.descriptiveNote so a key already withheld for the squashed-history reason
  // keeps that note rather than being overwritten with a second, unrelated one.
  const isGreenfieldWindow = summary.project_lifecycle === 'initial-build';
  const withheldEntries = isGreenfieldWindow
    ? squashWithheld.map(entry => (WITHHELD_WHEN_GREENFIELD_KEYS.has(entry.key) && !entry.descriptiveNote)
  ...
  if (isGreenfieldWindow) {
    ...
    if (densityIndex !== -1 && withheldEntries[densityIndex].direction !== 'informational') {
      withheldEntries[densityIndex] = withholdEntry(withheldEntries[densityIndex], WITHHELD_WHEN_GREENFIELD_NOTE);
    }
  }
...
module.exports = { buildMetricCatalog, buildGaugeSvgParts, topCommits, METRIC_GROUP_ORDER, groupForMetricKey };
