...
// The nine catalog entries measuring commit shape (a tenth, message_quality_pct, is already
// informational for an unrelated reason -- see its own entry below -- so withholding it again
// here would just duplicate, not compose with, that existing note). Under squashed history each
// analyzed unit is a whole pull request, not a commit, so none of these bands apply.
...
function withholdEntry(entry) {
  return {
    ...
    criticalBoundary: null,
    tier: undefined,
    descriptiveNote: WITHHELD_WHEN_SQUASHED_NOTE
  };
}
...
function buildMetricCatalog(summary, duplicates) {
  ...
  const withheldEntries = summary.history_granularity === 'squashed' || summary.history_granularity === 'unknown'
