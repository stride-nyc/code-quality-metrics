...
const THRESHOLDS = {
  ...
  //
  // n = 6 (stride-nyc/remote_retro, stride-nyc/dotnetdependencytracer, ziglang/zig,
  // denoland/deno, tiangolo/fastapi, sveltejs/svelte -- run
  // `node calibration/derive-bands.js --population greenfield-modern` to reproduce). Grown
  // from the original n = 2 by code-quality-metrics-vxr9: the first two repositories are both
  // members of the five-repo eval set this toolkit's own maintainer uses day to day
  // (calibration/observations.json's `greenfield-modern-eval-circularity` reservation), so
  // scoring either one's own commits against a band it helped define answers "does this
  // repository resemble itself," not "is this repository's practice unusual." The four added
  // repositories have no connection to this toolkit or its maintainer, which materially
  // reduces that circularity concern without eliminating it (two of six repositories behind
  // this band are still both rule-maker and rule-taker for themselves specifically) --
  // GitHub #84 tracks this and remains open. Still a much thinner sample than the n = 12
  // behind every band above.
  GREENFIELD_MODERN: {
    // sorted [16.67, 20.41, 24.00, 24.00, 52.00, 58.00]; p75 = 24.00 + (52.00-24.00)*0.75 =
    // 45.0 exactly, max = 58.00 (n = 6). Two repositories sit within 15% of the max:
    // stride-nyc/dotnetdependencytracer (58.00, its own value) and ziglang/zig (52.00).
    // Three-band -- this metric gains a critical bound for the first time in this population,
    // now corroborated across two independent repositories.
    LARGE_COMMITS_PCT: { healthy: 45, critical: 58 },

    // sorted [5.56, 16.33, 26.00, 28.00, 30.00, 56.00]; p75 = 28.00 + (30.00-28.00)*0.75 =
    // 29.5 -> rounds to 30, max = 56.00 (n = 6). Only stride-nyc/dotnetdependencytracer
    // (56.00) sits within 15% of the max. Two-band.
    SPRAWLING_COMMITS_PCT: { healthy: 30, critical: null },

    // sorted [2.04, 5.56, 8.00, 20.00, 74.00, 78.00], higher-is-better so healthy is p25:
    // 5.56 + (8.00-5.56)*0.25 = 6.17 -> rounds to 6, min = 2.04 (n = 6). Only tiangolo/fastapi
    // (2.04, its own value) sits within 15% of the min. Two-band.
    //
    // NOT CONSUMED. GREENFIELD_SUBSTITUTED_KEYS (lib/report.js) does not map this key, and
    // that omission is deliberate: at n = 6 this healthy value (6) is far more permissive than
    // the brownfield TEST_COVERAGE_RATE.healthy (23) above, and test/prod co-change
    // discipline is meant to keep scoring against the brownfield band regardless of
    // lifecycle -- see the long comment in lib/report.js explaining why, and
    // code-quality-metrics-96jv, which tracks making that omission harder to reverse by
    // accident. This value is recorded for completeness and provenance only.
    TEST_COVERAGE_RATE: { healthy: 6, critical: null },

    // sorted [4.00, 11.11, 12.00, 20.41, 22.00, 36.00]; p75 = 20.41 + (22.00-20.41)*0.75 =
    // 21.60 -> rounds to 22, max = 36.00 (n = 6). Only ziglang/zig (36.00) sits within 15% of
    // the max. Two-band.
    //
    // NOT CONSUMED, for the same reason and by the same mechanism as TEST_COVERAGE_RATE
    // above: at n = 6 this healthy value (22) is far more permissive than the brownfield
    // UNCOVERED_PROD_RATE.healthy (10) above, and GREENFIELD_SUBSTITUTED_KEYS does not map
    // this key. Recorded for completeness and provenance only.
    UNCOVERED_PROD_RATE: { healthy: 22, critical: null },

    // sorted [264.40, 376.50, 439.20, 502.60, 927.40, 1056.20]; p75 =
    // 502.60 + (927.40-502.60)*0.75 = 821.2 -> rounds to 820, max = 1056.20 -> rounds to 1060
    // (n = 6). Two repositories sit within 15% of the max: stride-nyc/remote_retro (927.40)
    // and stride-nyc/dotnetdependencytracer (1056.20, its own value). Three-band, as it
    // already was at n = 2, though the corroborating pair and the healthy bound both moved
    // (was healthy 1020).
    P90_LINES_CHANGED: { healthy: 820, critical: 1060 },

    // sorted [5.00, 7.40, 8.00, 9.10, 10.10, 13.10]; p75 = 9.10 + (10.10-9.10)*0.75 = 9.85 ->
    // rounds to 10, max = 13.10 (n = 6). Only stride-nyc/dotnetdependencytracer (13.10, its
    // own value) sits within 15% of the max. Two-band.
    P90_FILES_CHANGED: { healthy: 10, critical: null },

    // sorted [0, 0, 0, 0, 1.76, 23.15]; p75 = 0 + (1.76-0)*0.75 = 1.32 -> rounds to 1.5, max =
    // 23.15 (n = 6). Only tiangolo/fastapi (23.15, its own value) sits within 15% of the max
    // -- no second repository corroborates it, so no critical bound is reported despite the
    // much higher max than the n = 2 derivation saw. Two-band. Healthy is unchanged from the
    // n = 2 value (1.5): tiangolo/fastapi's own duplication (concentrated, real clones in
    // fastapi/security/*.py -- see its observations.json notes) sits well above the 75th
    // percentile of six, not at it, so it moves the max and the critical-corroboration
    // question without moving healthy.
    //
