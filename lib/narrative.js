...
function validateNarrative(bullets, payload, topCommits) {
  ...
  for (const bullet of bullets) {
    const namedEntries = payload.filter(entry => entry.label && bullet.toLowerCase().includes(entry.label.toLowerCase()));
    if (namedEntries.length !== 1) continue;
    const entry = namedEntries[0];
    let match;
