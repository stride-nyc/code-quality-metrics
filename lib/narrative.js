...
/**
 * Derive the provenance statement sent to the model from the population an entry was actually
 * scored against (code-quality-metrics-0b8f), rather than a hardcoded "six-repository" count
 * that goes stale as GitHub #84 grows the reference set. Reads entry.bandProvenance, the exact
 * field lib/report.js's substituteBand sets only when a verdict was scored against a
 * substituted band (currently THRESHOLDS.GREENFIELD_MODERN) -- the same field the rendered
 * band-chip and greenfield note already read, so the narrative can never assert a provenance
 * the tiles on the same page contradict. An entry with no bandProvenance was scored against the
 * default six-repository benchmark, so the note falls back to that wording when nothing in the
 * payload was substituted. Mixed runs are real (test/prod co-change and uncovered production
 * deliberately stay on the default band even when other tiles substitute -- see CLAUDE.md's
 * Project Lifecycle Detection section), so a substituted entry only changes the note to name
 * the substituted population in addition to, not instead of, the default one those two metrics
 * still use.
 * @param {Array<object>} payload
 * @returns {string}
 */
function describeBandProvenance(payload) {
  const substituted = payload.find(entry => entry.bandProvenance);
  if (!substituted) return BENCHMARK_PROVENANCE_NOTE;

  const { population, n } = substituted.bandProvenance;
  return `healthyBoundary/criticalBoundary values for most tiles here (change size, duplication) are quantiles of a ${population} reference band, not the usual six-repository benchmark, because this run looks like an early-stage project: just ${n} reference project${n === 1 ? '' : 's'}, so treat those verdicts as rougher than the rest (healthy is roughly the 75th percentile, or 25th for a higher-is-better metric; critical is the most extreme corroborated observation). Test/prod co-change and uncovered production still use the six-repository benchmark regardless. These are not validated outcome thresholds. An entry with criticalBoundary: null has no critical bound because its extreme rests on a single reference project -- do not state or imply one.`;
}

const NARRATIVE_SYSTEM_PROMPT = `You are writing the Findings section of a code quality drift report, for a reader who has never seen this tool before and will not ask what a term means -- if a sentence needs jargon explained, explain it in the same sentence. You are given a metric catalog and a list of top commits, both already computed by the report tool. Do not compute, estimate, or restate any number differently than given. Echo every number you reference exactly as given, to the same precision -- this includes a top commit's own prod_additions/prod_deletions/prod_files_count, which are exact facts about that one commit, not a rounded rate the way a catalog value is. Those three fields are already scoped to production code (excluded vendored/generated files are not counted), the same basis every verdict on this page uses -- describe a commit's size using them, and call them production lines or production files, never "total" (no whole-diff total is given to you, so there is nothing else to cite).

Five rules, in order of how much they matter to a first-time reader:

1. Lead with the consequence, not the metric name, for a positive finding exactly as much as a concern. Start each sentence with what is actually true or worth doing ("About a third of commits are too large to review line by line") and bring in the metric name and number as support, not the headline ("Large commits sit at 33%..." buries the point until the second half of the sentence). The same rule applies to a positive finding: lead with what the team does well ("Nearly two thirds of commits land tests alongside the production code they cover"), not the metric name first ("Test/prod co-change sits at 64%..." buries the point the same way). A reader scanning ten bullets should learn the finding, strength or concern, before they read ten metric names.
...
6. Never use an em dash (the character "—") or a double hyphen used as a pause (" -- "). This project's writing standard forbids both in any output, including this one. Use a period, comma, colon, semicolon, or parentheses instead.

7. State a finding and its own methodology caveat as separate sentences, never welded into one run-on sentence with "and" or a comma splice. A sentence reporting what the data shows, and a sentence qualifying how much to trust it, are two different jobs; do not make one sentence do both.
 ...
 *
 * hasScoredConcern (code-quality-metrics-0jqs) is whether the catalog itself has any entry
 * currently in warning or critical status. When it does not, nothing the model puts under
 * "concerns" can be a real concern -- there is nothing in warning/critical status for it to be
 * about -- so those entries are labeled "Note" instead of "Concern". A real dotnetdependencytracer
 * run had zero warning/critical metrics and 8 Positive bullets, but the model still wrote a
 * "concerns" entry stating there were no concerns, which this function used to label "Concern"
 * unconditionally: the report's one Concern bullet announced there were no concerns. Nothing in
 * validateNarrative below catches this case (the bullet cites no fabricated number and names no
 * informational label), so relabeling here, based on a fact already computed from the payload
 * rather than trusting the model's own framing, is the only reliable fix.
 * @param {object} parsed
 * @param {boolean} hasScoredConcern
 * @returns {string[]}
 */
function flattenNarrative(parsed, hasScoredConcern) {
  const groups = [
    ['Positive', parsed && parsed.positive_findings],
    [hasScoredConcern ? 'Concern' : 'Note', parsed && parsed.concerns],
...
/**
 * Locate every numeric literal in a string of prose, with its position, as a canonical
 * (percent-sign-stripped, thousands-comma-stripped) token in the same textual precision the
 * model wrote it. Stripping a thousands separator is a narrow, specific normalization -- the
 * payload never stores one (see buildNarrativePayload/formatValue), so a prose "14,803" and a
 * payload "14803" are the same number written two ways, not two different figures. This does
 * not generalize to stripping arbitrary punctuation from a number: a comma is only ever
 * removed here because NUMBER_PATTERN already anchored it inside a recognized
 * thousands-grouping shape. A fresh RegExp is built from NUMBER_PATTERN's own source/flags
 * rather than reusing the module-level instance directly, so repeated exec() calls here never
 * fight over that shared instance's lastIndex with any other caller.
 * @param {string} text
 * @returns {{ raw: string, index: number, token: string }[]}
 */
function matchNumbersWithPosition(text) {
  const re = new RegExp(NUMBER_PATTERN.source, NUMBER_PATTERN.flags);
  const matches = [];
  let match;
  while ((match = re.exec(text)) !== null) {
    matches.push({ raw: match[0], index: match.index, token: match[0].replace(/%$/, '').replace(/,/g, '') });
  }
  return matches;
}

/**
 * Extract every numeric literal from a string of prose as a canonical token (see
 * matchNumbersWithPosition above for the canonicalization rule). Positions are discarded here
 * because every current caller of this function -- collectAllowedNumbers -- only needs the set
 * of numbers a payload holds, never where in the (JSON) text they sit.
 * @param {string} text
 * @returns {string[]}
 */
function extractNumbers(text) {
  return matchNumbersWithPosition(text).map(m => m.token);
}

/**
 * A cited numeral is exempt from the presence check below when its immediate local context
 * marks it as something other than a claimed catalog measurement (code-quality-metrics-top7,
 * measured on a real five-repo regeneration: 2 of 4 rejections that run were false positives,
 * both from the presence check treating every numeral in the prose as a cited measurement).
 * Deliberately narrow -- two specific shapes, not a general attempt to classify what a number
 * in open-ended prose "means", which is exactly as much guesswork as the boundary-role and
 * informational-label checks below already decline to attempt in general:
 *
 *  - An HTTP status/error code ("...causing 500 errors"): immediately followed by
 *    "error"/"errors", which names a class of code, not a metric unit. No metric in this
 *    catalog is denominated in "errors", so this can never collide with a genuine citation.
 *
 *  - A ceiling a Recommended action itself proposes ("...exceeds roughly 300 lines of net
 *    change would...", or the equivalent "exceed"/"exceeding" bare and gerund forms, or
 *    "more than"/"over"/"above roughly N" -- code-quality-metrics-49ch: measured directly by
 *    generating the real narrative repeatedly against byte-identical input, all of these
 *    lead-ins turned up in real rejected output, e.g. "likely to exceed roughly 300" and
 *    "touching more than roughly 200"): introduced by one of these lead-ins immediately
 *    before "roughly", the shape a recommendation uses to float a NEW number it is
 *    suggesting, not report one the catalog already holds -- a genuine citation of an
 *    existing value is instead phrased as "is/at/of N" (still enforced by the loop below) or
 *    "boundary of N" (still enforced exactly by BOUNDARY_PHRASE further down). Deliberately
 *    narrower than exempting every approximated or comparative number: "roughly" is still
 *    required immediately before the number in every lead-in variant, since it is the one
 *    signal that distinguishes a proposed approximate ceiling from a real citation -- a bare
 *    "more than N" or "exceeds N" with no "roughly" hedge is left fully checked (see this
 *    function's own guard test), and rule 2 of the system prompt above tells the model to
 *    round a real citation for readability ("about 62%") using "about", not "roughly", so a
 *    rounded citation must still match the payload at that precision.
 * @param {string} bullet
 * @param {number} matchIndex
 * @param {string} rawMatch
 * @returns {boolean}
 */
function isExemptFromPresenceCheck(bullet, matchIndex, rawMatch) {
  const after = bullet.slice(matchIndex + rawMatch.length);
  if (/^\s*errors?\b/i.test(after)) return true;

  const before = bullet.slice(0, matchIndex);
  if (/\b(?:exceed(?:s|ing)?|more than|over|above)\s+roughly\s*$/i.test(before)) return true;
...
/**
 * Whether a bullet cites a specific scored (verdict-bearing) entry's own value together with
 * that same entry's own healthy or critical boundary, as literal numbers -- evidence the bullet
 * is about that metric even when it never names the metric's label in text
 * (code-quality-metrics-zuee). This is deliberately narrower than "both numbers appear somewhere
 * in the payload" (the general presence check earlier in this function already covers that
 * weaker case): it requires both numbers to trace back to the SAME entry, so a bullet cannot be
 * exempted merely because it happens to cite two unrelated numbers that belong to different
 * metrics. Only entries with a real boundary (healthyBoundary or criticalBoundary) are eligible
 * -- an entry with neither has no boundary this check could ever confirm.
 * @param {string} bullet
 * @param {Array<object>} scoredEntries - payload entries with verdict !== 'none'
 * @returns {boolean}
 */
function citesScoredEntryByValueAndBoundary(bullet, scoredEntries) {
  const cited = new Set(extractNumbers(bullet));
  return scoredEntries.some(entry => {
    const boundary = entry.healthyBoundary != null ? entry.healthyBoundary : entry.criticalBoundary;
    if (boundary == null || entry.value == null) return false;
    return cited.has(String(entry.value)) && cited.has(String(boundary));
  });
}

/**
 * Whether a bullet contains a literal em dash character (U+2014), or a double hyphen used as a
 * pause ("--"). This project's writing standard forbids both forms in any output, including
 * generated narrative bullets, and a model instruction alone is not a guarantee the model will
 * honor it (code-quality-metrics-wo8q, measured: 12 real em-dash characters and 7 double-hyphen
 * pauses in the regenerated 73V report's Findings section). A bare "--" has no legitimate use in
 * this narrative's prose (no CLI flags, no numeric ranges written that way), so a plain
 * substring check is safe here, the same way lib/narrative.js already treats a bare "--" as
 * unambiguous elsewhere in this file's own comments -- this project's own convention for
 * writing a pause is exactly the string being forbidden in generated output.
 * @param {string} bullet
 * @returns {boolean}
 */
function containsDisallowedDash(bullet) {
  return bullet.includes('—') || bullet.includes('--');
}
...
function validateNarrative(bullets, payload, topCommits) {
  ...
  for (const bullet of bullets) {
    for (const { token, index, raw } of matchNumbersWithPosition(bullet)) {
      if (isExemptFromPresenceCheck(bullet, index, raw)) continue;
  ...
  // code-quality-metrics-zuee: measured against a real 73V run. Naming a scored label in text
  // (the check above) is not the only way a bullet can prove its subject is a scored metric --
  // citing that metric's own value together with its own healthy/critical boundary is
  // number-for-number evidence of the same thing, and catches a bullet that paraphrases its
  // subject entirely (real rejected bullet: "Copy-pasted code has accumulated to 4.1% ... above
  // the benchmark's upper boundary of 1.5%", which never once says "Duplication density" but
  // does cite that exact metric's value and healthy boundary). scoredEntries is computed once,
  // outside the loop, since it does not depend on which bullet is being checked.
  const scoredEntries = payload.filter(entry => entry.verdict !== 'none');
  for (const bullet of bullets) {
    if (!bullet.startsWith('Concern: ')) continue;
    const lowerBullet = bullet.toLowerCase();
    if (scoredLabels.some(label => lowerBullet.includes(label))) continue;
    if (citesScoredEntryByValueAndBoundary(bullet, scoredEntries)) continue;
  ...
  // code-quality-metrics-wo8q: the project's writing standard forbids an em dash, and a double
  // hyphen used as a pause, in any output -- including generated prose. The system prompt asks
  // the model not to produce either, but a model instruction is not a guarantee, so this is
  // checked here too, the same defense-in-depth already applied to every other rule above.
  for (const bullet of bullets) {
    if (containsDisallowedDash(bullet)) {
      return { valid: false, reason: `contains an em dash or a double-hyphen pause, which this project's writing standard forbids: "${bullet}"` };
    }
  }
...
/**
 * Reduce each top commit to the fields safe for the narrative to cite (code-quality-metrics-j78y).
 * Strips the raw whole-diff totals (total_additions/total_deletions/files_changed) and keeps
 * only the exclusion-scoped production counts every verdict on the page already uses
 * (prod_additions/prod_deletions/prod_files_count), plus sha/message to identify the commit
 * and test_files_count so a bullet can still say "no test file touched alongside". A commit
 * dominated by an excluded vendored sync (e.g. a terraform lockfile refresh) can legitimately
 * still be a large or uncovered commit on its production lines alone -- it is not filtered out
 * here, only described by the correct, smaller number. Since these fields are also all the
 * model ever sees for a top commit, they double as the allowed-number set validateNarrative
 * checks a citation against: a bullet citing the old raw total (e.g. 9868) is rejected as
 * fabricated, the same way any other unlisted number already is.
 * @param {Array<object>} topCommits
 * @returns {Array<object>}
 */
function buildNarrativeTopCommits(topCommits) {
  return (topCommits || []).map(commit => ({
    sha: commit.sha,
    message: commit.message,
    prod_additions: commit.prod_additions,
    prod_deletions: commit.prod_deletions,
    prod_files_count: commit.prod_files_count,
    test_files_count: commit.test_files_count
  }));
}

/**
 * Make one narrative request/parse/flatten attempt: call the model, strip markdown fences,
 * parse the JSON, and flatten it into labeled bullets. Throws (never returns an empty array)
 * on an API error, a JSON parse failure, or an empty (all-groups-empty) response, so every
 * caller's try/catch already handles those the same way regardless of which attempt raised
 * them. Does not validate the bullets against the payload; that is the caller's job, since a
 * validation rejection (unlike the failures this function raises) is retried rather than
 * treated as fatal -- see generateFindingsNarrative's retry loop.
 * @param {object} client - Anthropic client instance
 * @param {Array<object>} payload - buildNarrativePayload's output
 * @param {Array<object>} narrativeTopCommits - buildNarrativeTopCommits' output
 * @returns {Promise<string[]>}
 */
async function requestNarrativeBullets(client, payload, narrativeTopCommits) {
  const response = await client.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `${describeBandProvenance(payload)}\n\nMetric catalog (JSON):\n${JSON.stringify(payload)}\n\nTop commits (JSON):\n${JSON.stringify(narrativeTopCommits)}`
    }]
  });
  ...
  const hasScoredConcern = payload.some(entry => entry.status === 'warning' || entry.status === 'critical');
  const bullets = flattenNarrative(parsed, hasScoredConcern);
...
/**
 * Prepend a bullet stating plainly that the Findings section below is the deterministic
 * fallback list, not an AI-generated narrative, and a brief, generic reason why
 * (code-quality-metrics-49ch). flight-info-spike's own report varied silently between two runs
 * against byte-identical input, once rendering this fallback and once the full narrative, with
 * nothing in the rendered report telling a reader which one they were looking at -- the reason
 * given here is deliberately generic (never the model's own rejected text or a raw err.message)
 * rather than reused from validateNarrative's own reason string, matching
 * code-quality-metrics-zuee's precedent: that internal diagnostic belongs in the run's logs
 * (console.error/console.log, unchanged), not quoted verbatim into rendered report content.
 * Returned as a bullet, the same shape every other Findings entry already is, rather than a
 * separate return value or flag: lib/report-template.js's renderFindings already renders
 * whatever array of strings it is given with no changes needed there.
 * @param {Array<object>} catalog - already-computed catalog from buildMetricCatalog
 * @param {string} reason - brief, generic clause completing "...not an AI-generated narrative: <reason>."
 * @returns {string[]}
 */
function withFallbackNotice(catalog, reason) {
  return [`Note: This Findings section is the deterministic fallback list, not an AI-generated narrative: ${reason}.`, ...fallbackFindings(catalog)];
}
 ...
 *
 * A validation rejection is retried, up to CONFIG.NARRATIVE_MAX_ATTEMPTS total attempts,
 * before falling back (code-quality-metrics-49ch): diagnosed directly by generating the real
 * narrative repeatedly against byte-identical input, which measured a real, non-negligible
 * false-positive rate in validateNarrative's checks (the model phrasing a legitimate point in
 * a way the checks cannot statically distinguish from a fabrication) that no single regex
 * exemption can fully close, since the model rephrases differently on every call. A second,
 * independently-generated attempt is far less likely to repeat the same rejected phrasing than
 * the first attempt was to produce it. This is deliberately NOT applied to a genuine API/parse
 * error (requestNarrativeBullets throwing): the Anthropic SDK's own default maxRetries already
 * retries a transient API failure before this function's catch block ever sees one, so retrying
 * that case again here would be redundant, and a non-transient error (a malformed request, a
 * hard account-level failure) would not be fixed by retrying it either.
...
async function generateFindingsNarrative(client, catalog, topCommits) {
  if (!client) return withFallbackNotice(catalog, 'no ANTHROPIC_API_KEY is configured for this run');

  const payload = buildNarrativePayload(catalog);
  const narrativeTopCommits = buildNarrativeTopCommits(topCommits);

  try {
    for (let attempt = 1; attempt <= CONFIG.NARRATIVE_MAX_ATTEMPTS; attempt++) {
      const bullets = await requestNarrativeBullets(client, payload, narrativeTopCommits);
      const validation = validateNarrative(bullets, payload, narrativeTopCommits);
      if (validation.valid) return bullets;

      // Logged rather than thrown -- see code-quality-metrics-ll1's decision: fail visibly, not
      // silently -- but code-quality-metrics-zuee found the rejection reason itself rendering
      // in the report's Findings section, quoting the model's full rejected paragraph as the
      // first thing a reader saw: an internal consistency-check diagnostic, not report content,
      // and one that left a genuine warning (the paragraph it rejected) looking discredited
      // rather than explained. console.error (stderr) keeps the diagnostic available for
      // debugging without it ever reaching the rendered bullets returned below.
      const willRetry = attempt < CONFIG.NARRATIVE_MAX_ATTEMPTS;
      console.error(`ℹ️  Findings narrative rejected (attempt ${attempt}/${CONFIG.NARRATIVE_MAX_ATTEMPTS}), ${willRetry ? 'retrying' : 'using templated fallback'}: ${validation.reason}`);
    }

    return withFallbackNotice(catalog, 'the generated narrative failed an internal consistency check on every attempt this run; see the run\'s logs for detail');
  } catch (err) {
    console.log(`ℹ️  Findings narrative unavailable, using templated fallback: ${err.message}`);
    return withFallbackNotice(catalog, 'narrative generation failed this run; see the run\'s logs for detail');
  }
}

module.exports = { generateFindingsNarrative, NARRATIVE_SYSTEM_PROMPT, buildNarrativePayload, buildNarrativeTopCommits, describeBandProvenance, validateNarrative };
