 ...
 *    change would..."): introduced by "exceeds roughly", the shape a recommendation uses to
 *    float a NEW number it is suggesting, not report one the catalog already holds -- a
 *    genuine citation of an existing value is instead phrased as "is/at/of N" (still enforced
 *    by the loop below) or "boundary of N" (still enforced exactly by BOUNDARY_PHRASE further
 *    down). Deliberately narrower than exempting every approximated number: "roughly" alone,
 *    without a preceding "exceeds", is left fully checked, since rule 2 of the system prompt
 *    above tells the model to round a real citation for readability ("about 62%"), and that
...
function isExemptFromPresenceCheck(bullet, matchIndex, rawMatch) {
  ...
  if (/\bexceeds\s+roughly\s*$/i.test(before)) return true;
