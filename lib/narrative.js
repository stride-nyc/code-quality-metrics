...
async function generateFindingsNarrative(client, catalog, topCommits) {
  ...
  try {
    ...
    if (!validation.valid) {
      ...
      // does have something specific to say, so it is still logged -- see
      // code-quality-metrics-ll1's decision: fail visibly, not silently -- but
      // code-quality-metrics-zuee found the rejection reason itself rendering in the report's
      // Findings section, quoting the model's full rejected paragraph as the first thing a
      // reader saw: an internal consistency-check diagnostic, not report content, and one
      // that left a genuine warning (the paragraph it rejected) looking discredited rather than
      // explained. console.error (stderr) keeps the diagnostic available for debugging without
      // it ever reaching the rendered bullets returned below.
      console.error(`ℹ️  Findings narrative rejected, using templated fallback: ${validation.reason}`);
