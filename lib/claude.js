...
/**
 * Analyze a set of files for semantic duplicate patterns using Claude.
 * Returns empty array when client is null or on API/parse errors.
...
async function analyzeDuplicatesWithClaude(client, filePaths, staticFindings) {
  ...
  const fileSnippets = filePaths.map(p => {
    try {
...
module.exports = { CLAUDE_SYSTEM_PROMPT, DUPLICATE_SYSTEM_PROMPT, getAnthropicClient, selectClaudeCommits, analyzeWithClaude, analyzeDuplicatesWithClaude };
