...
/**
 * Converts one jscpd firstFile/secondFile entry's `name` from the absolute path
 * `--absolute` produces back to a path relative to process.cwd() (the analyzed
 * repository's root in every real caller), preserving every other field
 * (start/end/startLoc/endLoc) unchanged (code-quality-metrics-3i6e).
 *
 * Without this, either both members of a pair would render as an ambiguous bare
 * basename (jscpd's default, which truncates every file to its basename with no
 * directory prefix at all, verified live even when the basename is unique in the
 * scanned set), or -- if left at --absolute's raw output -- both would leak the
 * operator's absolute local filesystem path into a report that is explicitly
 * scrubbed and shared outside the client context.
 * @param {{name: string, start: number, end: number}} fileEntry
 * @returns {{name: string, start: number, end: number}}
 */
function toRepoRelativeFileEntry(fileEntry) {
  if (!fileEntry) return fileEntry;
  return { ...fileEntry, name: path.relative(process.cwd(), path.resolve(fileEntry.name)) };
}
...
function runJscpdPass(filePaths, minLines, minTokens) {
  ...
  try {
    ...
    // --absolute (code-quality-metrics-3i6e): jscpd's own default report names every
    // file by bare basename only, with no directory prefix at all -- verified live,
    // this holds even when a basename is unique in the whole scanned set, not only when
    // it collides with another file. A repository with more than one file sharing a
    // basename (three files named main.tf is a real, measured case) then renders an
    // unresolvable "duplicates main.tf" the reader cannot act on. --absolute makes the
    // report carry the full path, which runJscpdPass below normalizes back to a
    // repo-relative path so both members of a pair render on the same, distinguishable
    // basis.
    const cmd = [
      'npx jscpd',
      `--min-lines ${minLines}`,
    ...
    const findings = (report.duplicates || []).map(d => ({
      firstFile:  toRepoRelativeFileEntry(d.firstFile),
      secondFile: toRepoRelativeFileEntry(d.secondFile),
      lines:  d.lines,
      tokens: d.tokens
    }));
