...
it to two-band instead (see that script's `deriveBand` comment). Because every one of these
top-level, brownfield bands is currently two-band, no metric scored against them can render a
"critical" verdict right now; `classifyDoraArchetype`'s `legacy-bottleneck` and
`foundational-challenges` archetypes are consequently unreachable until a future re-measurement
restores a three-band metric here (see the DORA Archetype section below). This claim is scoped
to this table deliberately: `THRESHOLDS.GREENFIELD_MODERN` (see "Project Lifecycle Detection"
below) is a separate, non-pooled band set that already carries a three-band metric of its own
(`P90_LINES_CHANGED`, since that population's adoption at n=2, joined by `LARGE_COMMITS_PCT` at
n=6), but `classifyDoraArchetype` reads only the brownfield keys above and never
`GREENFIELD_MODERN`, so neither greenfield three-band metric makes either archetype reachable.
